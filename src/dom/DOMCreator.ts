...
export default class DOMCreator {
   constructor(readonly containerName: string, readonly options: Options) {
      ...
      var mainElement = document.getElementById(containerName)!;
   ...
   getElement<T extends HTMLElement = HTMLElement>(id: string) : T {
      ...
      const element = document.getElementById(this.options.external.prefix + id);
    ...
    * @param id 
    */
   getElement$<T extends HTMLElement = HTMLElement>(id: string) : JQuery<T> {
      return JQuery(this.getElement<T>(id));
   }
}
