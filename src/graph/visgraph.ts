...
export function cleanNode(
  nodeIn: DirtyNode,
  NODE_STYLES: Record<string, INodeStyle>
): CleanNode {
  ...
  if (node.style !== undefined && style) {
    ...
    const hasPreviewOrMath =
      (node.previewhtml && node.previewhtml.length > 10) ||
    ...
    if (node.shapeProperties === undefined) {
      (node.shapeProperties as unknown) = {};
    }
 ...
 * @param NODE_STYLES
 */
export function cleanEdge(
  edgeIn: DirtyEdge,
  ARROW_STYLES: Record<string, IArrowStyle>
): CleanEdge {
...
export type IPositionWithId = Pick<CleanNode, 'id'> &
  Partial<Pick<CleanNode, 'x' | 'y'>>;
